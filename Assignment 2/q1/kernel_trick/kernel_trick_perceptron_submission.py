...
def kernelize(X, Y):
    ...
    row1 = X[:,0:1]
    row2 = X[:,1:2]
    row3 = np.square(row1)+np.square(row2)
    row4 = row1+row2
    row6 = np.sqrt(row3)

    X2 = np.append(1.3*row1,row4, axis = 1)
    kernelX = np.append(X2, row6, axis = 1) 
    """
    ===========================================================================
    """
...
def perceptron(X, Y):
    ...
    # YOUR CODE GOES HERE
    clf = Perceptron(penalty='L1',alpha=0.01,max_iter=10000,tol = None)
    clf.fit(kernelX,Y)
