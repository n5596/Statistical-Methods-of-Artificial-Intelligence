...
def SVM(train_data,
    ...
    # YOUR CODE GOES HERE
    kernel = 'rbf'
    svmmodel = svm.SVC(C=13, kernel=kernel, gamma=0.1, coef0=0,max_iter = -1)
    svmmodel.fit(train_data,train_labels)
    train_predictions = svmmodel.predict(train_data)
    """
    ==========================================================================
    """
...
#    print "Training Accuracy: %.4f" % (train_accuracy)
    ...
    # YOUR CODE GOES HERE
    test_predictions = svmmodel.predict(test_data)
...
#            "samples: %d" % (fold, len(train_indices), len(test_indices))
    ...
    # YOUR CODE GOES HERE
    x = 4*[0] # avgs per column
    nelem = float(5)
    for col in xrange(4):
        for row in xrange(5):
            x[col] += np.mean(accumulated_metrics[row][col])
        x[col] /= nelem
    #print(x)
    print("%.4f, %.4f, %.4f, %.4f") %(x[0],x[1],x[2],x[3])
    """
    ==========================================================================
    """
